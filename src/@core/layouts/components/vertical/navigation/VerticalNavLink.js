...
const VerticalNavLink = ({
  ...
  const isNavLinkActive = () => {
    if (
      (Array.isArray(item.path) && item.path.includes(router.pathname)) ||
      (!Array.isArray(item.path) && router.pathname === item.path) ||
    ...
    <CanViewNavLink navLink={item}>
      ...
      >
        <MenuNavLink
          ...
          href={item.path === undefined ? '/' : Array.isArray(item.path) ? item.path[0] : item.path}
